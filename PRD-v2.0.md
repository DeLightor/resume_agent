...
**US-37：投递追踪与岗位-版本关联（D1）**
**As a** 多方向求职者，**I want** 每次投递都关联到当时使用的简历版本和岗位上下文，**so that** 我能持续跟进，也能回看某个结果对应哪版简历。

验收标准：
- [x] 从已选简历节点创建投递记录，服务端冻结节点版本和完整内容快照；后续编辑或回收节点不得改变该快照
- [x] 保存公司、岗位、职位链接、结构化 JD、状态、下一步、跟进时间和备注；不自动投递、访问招聘网站或接入邮箱/日历
- [x] 看板按状态展示记录并标记未结束的逾期跟进；状态文字与颜色同时可见
- [x] 详情显示不可变投递版本、JD 摘要和追加式时间线；状态变化包含前后状态和说明
- [x] 记录编辑、删除和恢复采用乐观锁，陈旧页面保存时提示刷新，不能静默覆盖
- [x] 删除进入回收站，30 天内可恢复，默认看板不显示已删除记录
...
- 求职看板/投递时间线（竞品红海，版本树+Agent 已是差异化；该限制不适用于后续独立立项的 US-37）
- 模板市场/自定义模板导入
