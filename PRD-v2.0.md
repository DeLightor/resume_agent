...
> 文档版本：v2.0　|　状态：DRAFT（待评审）　|　日期：2026-09-08　|　前置：PRD.md v1.4（US-1~US-26 已交付）
...
验收标准：
- [ ] 定义 `Tool` 协议：name / description / parameters schema / execute
- [ ] 首批封装 8 个工具：retrieve_knowledge、parse_jd、analyze_gap、read_node、write_node、render_preview、export_pdf、web_search
- [ ] Agent Loop：deepseek function calling 多轮循环，最大轮数可配（默认 8）
- [ ] 每轮的工具调用与结果记录为结构化 trace（SQLite 新表 agent_traces）
- [ ] ask_user 工具：Agent 主动暂停等待用户输入，会话状态可恢复
- [ ] 单元测试覆盖每个工具的 schema 校验与执行

**US-28：SSE 流式与 Agent 行为面板（A2）**
**As a** 求职者，**I want** 看到 Agent 正在做什么（思考、检索、审查），**so that** 我信任并理解产出结果。

验收标准：
- [ ] 后端 `POST /api/agent/chat` 返回 `text/event-stream`
- [ ] 事件类型：`thinking` / `tool_call` / `tool_result` / `review` / `draft` / `done` / `error`
- [ ] 前端「Agent 行为面板」：时间线渲染每步动作，工具调用可展开看入参出参
- [ ] 生成过程从「转圈 30s」变为「边看边等」
- [ ] SSE 断线自动重连，会话不丢失
...
验收标准：
- [ ] 右栏新增对话入口，支持：「帮我针对这个 JD 优化当前节点的项目经历」「我的经历太少怎么办」「把这段改得更量化」
- [ ] Agent 能结合当前选中节点、已上传 JD、Gap 报告作为对话上下文
- [ ] ask_user 触发时对话内出现输入卡片（如「这段项目你实际的贡献是什么？」）
- [ ] 对话历史存 SQLite，刷新可恢复
- [ ] 传统按钮流并存不删
...
验收标准：
- [ ] Reviewer 使用独立上下文（不复用 Orchestrator 的对话历史）
- [ ] 检查项：知识库边界（素材外的内容标黄）、套话检测、JD 关键词覆盖、量化数字无来源标记
- [ ] 不合格项带修改意见打回，最多 2 轮重写，仍不合格则如实展示问题
- [ ] 审查意见随 diff 一起展示（「为什么这样改」）
- [ ] 既有「反思」逻辑迁移并入 Reviewer，删除旧实现

**US-31：解析确认流（B2）**
**As a** 求职者，**I want** 上传旧简历后逐字段确认提取结果再入库，**so that** 错误数据不进版本树和知识库。

验收标准：
- [ ] /parse 改为两阶段：提取（异步任务）→ 确认（用户审阅）→ 入库
- [ ] 提取结果每个字段带置信度标记（高/中/低），低置信度字段高亮
- [ ] 确认界面支持字段级编辑修正
- [ ] 知识库个人信息覆盖逻辑改为可见提示（「检测到知识库已有电话，是否覆盖？」）
- [ ] MinerU 解析改异步任务 + SSE 进度推送，超时自动降级本地解析器并提示
- [ ] 确认后的数据才写入 resume_chunks（错误数据不再污染 RAG）

**US-32：编辑保护三件套（B3）**
**As a** 求职者，**I want** 编辑不丢字、AI 不覆盖我的精调、误删可撤销，**so that** 我敢放心编辑。

验收标准：
- [ ] debounce autosave（800ms）替代纯 blur 保存，切节点/刷新前 flush
- [ ] 节点 content_json 增加乐观锁 version 字段，保存冲突时提示而非静默覆盖
- [ ] AI 生成/重生成结果默认以 **diff 预览卡片** 呈现（逐字段接受/拒绝），复用 v1.3 合并交互
- [ ] 节点内编辑历史：每次写入记录轻量 commit（时间 + 摘要 + 完整 content_json 快照，保留最近 20 条）
- [ ] undo/redo 快捷键（⌘Z/⌘⇧Z）基于 commit 历史
- [ ] 节点删除改软删除 + 回收站（30 天），替代级联硬删除
...
验收标准：
- [ ] upstream_changes 检测范围扩展到 experience/projects/skills（段落级 + 条目级 diff）
- [ ] 合并粒度：条目级（单条 bullet、单个项目）接受/拒绝
- [ ] master 内容变更后子节点标记更新（前端 SSE 推送，替代轮询）
- [ ] 合并交互复用 v1.3 Diff 视图范式
- [ ] 检测算法有单元测试（同字段双向修改冲突场景）

**US-34：预览分页与一致性校验（C1）**
**As a** 求职者，**I want** 预览里看到 A4 分页线和总页数，**so that** 导出前就知道是不是一页纸。

验收标准：
- [ ] ResumePreview 增加 A4 分页指示线与页码
- [ ] 页数超出 1 页时提示（可配置目标页数）
- [ ] 集成测试：预览分页断点与 PDF 分页断点一致率抽检（同内容同模板对比）

**US-35：Agent 记忆（C2）**
**As a** 求职者，**I want** Agent 记住我的偏好（如「不要动词开头堆砌」「我坚持两页纸」），**so that** 越用越顺手。

验收标准：
- [ ] SQLite 新表 agent_memories（类型：偏好/纠错/风格样例）
- [ ] Reviewer 审查时注入用户偏好记忆
- [ ] 对话中「以后都……」类指令自动提炼为记忆，用户可在设置里查看/删除
- [ ] 记忆注入有 token 上限控制（Top-K 相关记忆）

**US-36：应届生追问式冷启动（C3）**
**As a** 应届生，**I want** Agent 像师兄一样追问我的课程项目/比赛/社团，**so that** 零散经历变成简历语言。

验收标准：
- [ ] 「素材挖掘」对话模式：逐条追问背景/动作/结果/量化，产出 STAR 结构素材
- [ ] 挖掘出的素材直接进知识库（带来源标记「对话挖掘」）
- [ ] 追问模板覆盖：课程设计、竞赛、科研、社团、实习五类
- [ ] 中途可保存进度，下次继续
- [ ] 素材去重（向量相似度查重）
...
| 阶段 | 内容 | 依赖 |
|------|------|------|
| **v2.0-alpha** | US-27 工具注册表 + Agent Loop + US-28 SSE/行为面板 | 无（地基） |
| **v2.0-beta** | US-29 对话工作台 + US-30 Reviewer 双审 | alpha |
| **v2.0-rc** | US-31 解析确认流 + US-32 编辑保护三件套 | 无强依赖，可与 beta 并行 |
| **v2.0** | US-33 内容级合并 + US-34 分页预览 + 发布 | rc |
| **v2.1** | US-35 Agent 记忆 + US-36 应届生冷启动 | v2.0 对话数据积累 |
