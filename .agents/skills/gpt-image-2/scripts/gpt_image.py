...
DEFAULT_BASE_URL = "https://yybb.codes/v1"
...
def load_yybb_config_key() -> str | None:
    configured_path = os.environ.get("YYBB_CONFIG_FILE")
    config_path = (
        Path(configured_path).expanduser()
        if configured_path
        else Path.home() / ".codex" / "secrets" / "yybb.json"
    )
    ...
    try:
        ...
        raise SystemExit(f"Unable to read YYBB config: {config_path}") from exc
...
def extract_image_records(node: Any, *, event_name: str | None = None) -> List[Dict[str, Any]]:
    ...
    def walk(value: Any, path: str) -> None:
        if isinstance(value, dict):
            ...
            if isinstance(value.get("url"), str) and value["url"].startswith(("http://", "https://", "data:")):
                records.append(
                    {
                        "kind": "partial" if event_name and "partial" in event_name.lower() else "final",
                        "url": value["url"],
...
def decode_image_record(record: Dict[str, Any]) -> Dict[str, Any]:
    if "url" in record:
        try:
            req = request.Request(record["url"])
            with request.urlopen(req, timeout=60) as resp:
                image_bytes = resp.read()
        except Exception as exc:  # noqa: BLE001
            raise SystemExit(f"Failed to download image from {record['url']}.") from exc
    else:
        try:
...
def common_settings(args: argparse.Namespace, dotenv_values: Dict[str, str]) -> Dict[str, Any]:
    ...
    if not api_key:
        api_key = resolve_value(None, "YYBB_API_KEY", dotenv_values)
    if not api_key:
        api_key = load_yybb_config_key()
    if not api_key and not args.dry_run:
        raise SystemExit(
            "Missing OPENAI_API_KEY or YYBB_API_KEY. Set one in the environment, a .env file, "
            "or ~/.codex/secrets/yybb.json."
        )
