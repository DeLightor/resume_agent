...
| --- | --- | --- | --- |
| `OPENAI_API_KEY` | no | none | API key for the OpenAI-compatible endpoint; takes precedence |
| `YYBB_API_KEY` | no | none | YYBB API key used when `OPENAI_API_KEY` is unset |
| `OPENAI_BASE_URL` | no | `https://yybb.codes/v1` | Base URL, must include `/v1` |
...
1. CLI flags
2. `OPENAI_API_KEY`, then `YYBB_API_KEY`
3. `.env` file
4. `~/.codex/secrets/yybb.json` (`api_key`) for YYBB fallback
5. Built-in defaults
