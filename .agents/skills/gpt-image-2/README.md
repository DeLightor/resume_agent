...
- Python 3.10+
- An OpenAI-compatible endpoint that serves `gpt-image-2` (this installed variant defaults to `https://yybb.codes/v1`).
- For `responses`, a text-capable Responses model such as `gpt-5.4` when using the hosted `image_generation` tool.
- `OPENAI_API_KEY`, `YYBB_API_KEY`, or `~/.codex/secrets/yybb.json` for the configured YYBB key.
